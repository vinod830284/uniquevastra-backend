...
async function main() {
  ...
  for (const c of categoriesData) {
    ...
    const created = await prisma.category.upsert({
      where: { slug: c.slug },
      update: { name: c.name, parentId, status: CategoryStatus.ACTIVE, sortOrder: c.sort },
      create: {
        name: c.name,
        slug: c.slug,
  ...
  for (const col of collectionsData) {
    const created = await prisma.collection.upsert({
      where: { slug: col.slug },
      update: { name: col.name, status: CollectionStatus.ACTIVE, isFeatured: col.isFeatured },
      create: {
        name: col.name,
        slug: col.slug,
